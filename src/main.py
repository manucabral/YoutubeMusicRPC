...
class YoutubeMusicRPC:
    ...
    def _message(self, msg: str, type: str = 'info', log: bool = False) -> None:
        ...
        data = keys[type]
        print(
            f'{color.WHITE}[{data[0]}{data[1]}{color.WHITE}] {msg}{color.RESET}')
        if log:
            self._write_log(msg)
    ...
    def _get_browsers(self) -> []:
        ...
        for browser in BROWSERS:
            ...
            browsers.append(browser) if browser[0].lower(
            ) in result.lower() else None
        return browsers
    ...
    def _check_browsers(self) -> None:
        ...
        self.browser = Browser(
            name=browser[0], fullname=browser[1], process=browser[2], base=browser[3], path=browser[4])
        self._message(
            f'{MESSAGES[3][self.lang]} {self.browser.fullname} ({self.browser.process})', type='success')

    def _check_system_lang(self) -> None:
        self._message(MESSAGES[4][self.lang], type='info')
        lang = co('powershell get-uiculture', universal_newlines=True,
                  stderr=PIPE).split('\n')[3][17:].split('           ')
        self.system_lang = (lang[0], lang[1][1:])
        if not lang[0]:
            raise SystemLanguageNotFound()
        self._message(
            f'{MESSAGES[5][self.lang]} {self.system_lang[1]}', type='success')
    ...
    def _display_logo(self):
        print(color.RED, BANNER[0], color.WHITE, BANNER[1],
              color.LIGHTCYAN_EX, BANNER[2], color.RESET)
    ...
    def _get_buttons(self, details: str, url: str) -> []:
        ...
        buttons.append(
            {"label": MESSAGES[9][self.lang], "url": "https://github.com/manucabral/YoutubeMusicRPC"})
        return buttons
...
if __name__ == '__main__':
    app = YoutubeMusicRPC(client_id='828083725790609459', lang='es')
    app.run()
