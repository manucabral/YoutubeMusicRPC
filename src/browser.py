...
class Browser:
  def __init__(self, **args):
    ...
    self.path = getenv('LOCALAPPDATA') + args['path']
  ...
  def current_website(self) -> tuple:
    ...
    result = self._execute_query(self.temp_path, 'SELECT title, url, datetime(last_visit_time / 1000000 + (strftime("%s", "1601-01-01")), "unixepoch", "localtime") from urls ORDER BY last_visit_time DESC limit 1')
